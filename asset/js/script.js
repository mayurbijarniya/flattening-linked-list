...
function changeGridDim() {
  ...
  const colWidth =
    (dimes.width - scrollBarWidth - cmpStules.padding.split("px", 1)[0] * 2) /
    totalCols;
...
function changeNodesSize() {
  ...
  if (allFullNodes.length > 2) {
    //exclude 1 and last child
...
function getValsArray(valsStr) {
  const aa = Array.from(JSON.parse("[" + valsStr + "]")).flat(1000);
...
btnReverse.addEventListener("click", () => {
  ...
  for (let i = fullNodesRefs.length - 2; i >= 1; i--) {
    ...
    nodesContainer.lastElementChild.insertAdjacentElement(
      "beforeBegin",
      curNode
    );
  }
